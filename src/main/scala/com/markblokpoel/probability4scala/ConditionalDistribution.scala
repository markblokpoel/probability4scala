...
/**
  ...
  */
case class ConditionalDistribution[A, B](
    domainV1: Set[A],
    domainV2: Set[B],
    distribution: Map[(A, B), BigNatural]) {
  ...
  def pr(conditional: Conditional1[A, B]): BigNatural =
    distribution((conditional.value, conditional.condition))
  ...
  def pr(conditional: B): Distribution[A] = {
    ...
    if (d.sum == 0) d
      ...
      this * domainV2.singleValueDistribution(conditional) / prConditional(
        conditional)
  }
  ...
  def prValue(value: A): BigNatural = domainV2.foldLeft(0.toBigNatural) {
    (acc: BigNatural, conditional: B) =>
      acc + pr(value | conditional)
  }
  ...
  /**
    ...
    * @return
    */
  def prConditional(conditional: B): BigNatural =
    domainV1.foldLeft(0.toBigNatural) { (acc: BigNatural, value: A) =>
      acc + pr(value | conditional)
    }
  ...
  def marginalLikelihood: Distribution[B] =
    Distribution(domainV2,
                 domainV2.map(value => value -> prConditional(value)).toMap)
  ...
  /**
    ...
    * @return
    */
  def *(prior: Distribution[B]): Distribution[A] =
    Distribution(
      domainV1,
      domainV1
        .map((value1: A) =>
  ...
  def --(other: Distribution[A]): ConditionalDistribution[A, B] = {
    require(domainV1 == other.domain,
            "subtraction for distributions requires the same domains")

    val newDistribution = (for (d1 <- domainV1; d2 <- domainV2)
      yield
  ...
  def *(scalar: BigNatural): ConditionalDistribution[A, B] =
    ConditionalDistribution(domainV1,
                            domainV2,
                            distribution.mapValues(_ * scalar))
  ...
  def +(other: ConditionalDistribution[A, B]): ConditionalDistribution[A, B] = {
    val sumDistribution = (for (value1 <- domainV1; value2 <- domainV2) yield {
      (value1, value2) -> (distribution((value1, value2)) + other.distribution(
        (value1, value2)))
    }).toMap
  ...
  def pr(conditional: Conditional2[A, B]): BigNatural =
    domainV2
      .map(condition =>
        pr(conditional.value | condition) * conditional.prior.pr(condition))
      .fold(BigNatural(0))((acc: BigNatural, p: BigNatural) => acc + p)
  ...
  def bayes(prior: Distribution[B]): ConditionalDistribution[B, A] = {
    val newDistribution: Set[((B, A), BigNatural)] =
      domainV2.flatMap(condition => {
        domainV1.map(value => {
...
//    }).sum
  ...
  /**
    ...
    */
  def sum: BigNatural =
    distribution.values.fold(BigNatural(0))((acc: BigNatural, p: BigNatural) =>
      acc + p)

  def exp: ConditionalDistribution[A, B] =
    ConditionalDistribution(domainV1, domainV2, distribution.mapValues(_.exp))

  def log: ConditionalDistribution[A, B] =
    ConditionalDistribution(domainV1, domainV2, distribution.mapValues(_.log))
  ...
  /**
    ...
    *      [[https://en.wikipedia.org/wiki/Softmax_function]].
    */
  def softmax(beta: BigNatural,
              costs: Distribution[A]): ConditionalDistribution[A, B] = {
...
//    println(norm)
  ...
  def cpt(): Unit = {
    ...
    println(
      " " * (maxStrLen1 + 1) + domainV2
        .map(d2 => d2.toString + " " * (maxStrLen2 - d2.toString.length + 1))
        .mkString)
    domainV1.foreach { d1 =>
      println(
        d1.toString + " " * (maxStrLen1 - d1.toString.length + 1) +
          domainV2.toVector
            .map(d2 =>
              f"${pr(d1 | d2).doubleValue()}%1.4f" + " " * (maxStrLen2 - f"${pr(
                d1 | d2).doubleValue()}%1.4f".length + 1))
            .mkString
      )
    }
  }
...
case object ConditionalDistribution {
  def apply[A, B, X: ClassTag](
      domain1: Set[A],
      domain2: Set[B],
      distribution: Map[(A, B), Double]): ConditionalDistribution[A, B] = {
