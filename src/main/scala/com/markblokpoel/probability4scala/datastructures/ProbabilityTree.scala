...
case class ProbabilityTree[A](distribution: Distribution[A]) {
  ...
  private def constructPTree(values: Vector[A],
                             probabilities: Vector[BigNatural],
                             p: BigNatural): PTree[A] = {
    val (leftValues, rightValues) = values.splitAt(values.length / 2)
    val (leftProbabilities, rightProbabilities) =
      probabilities.splitAt(probabilities.length / 2)
    val leftSum =
      leftProbabilities.fold(BigNatural(0))((acc: BigNatural, p: BigNatural) =>
        acc + p)
