...
object Implicits {
  ...
  implicit class ImplDistributionConstruct[A](domain: Set[A]) {
    ...
    def uniformDistribution: Distribution[A] = {
      val distribution = domain
        .map(value =>
          value -> BigNatural(1.0) / BigNatural(domain.size.doubleValue()))
        .toMap
    ...
    def singleValueDistribution(value: A): Distribution[A] = {
      require(domain.contains(value),
              "Cannot construct distribution for value outside the domain.")
      Distribution(domain,
                   domain.map(v => v -> (if (v == value) 1.0 else 0.0)).toMap)
    }
  }
  ...
  implicit class ImplConditional[A](value: A) {
    ...
    /**
      ...
      * @return
      */
    def |[B](condition: Distribution[B]): Conditional2[A, B] =
      Conditional2(value, condition)
  }

  implicit class ImplDouble(double: Double) {

    /** Converts Double to BigNatural. */
    def toBigNatural: BigNatural =
      BigNatural(double, double.isPosInfinity, double.isNegInfinity)

    def *[A, B](
        cd: ConditionalDistribution[A, B]): ConditionalDistribution[A, B] =
      cd * double.toBigNatural
    ...
    def -(nat: BigNatural): BigNatural = BigNatural(double) - nat
    def /(nat: BigNatural): BigNatural = BigNatural(double) / nat
    def /%(nat: BigNatural): (BigNatural, BigNatural) =
      BigNatural(double) /% nat
    ...
    def remainder(nat: BigNatural): BigNatural =
      BigNatural(double) remainder nat
  }
