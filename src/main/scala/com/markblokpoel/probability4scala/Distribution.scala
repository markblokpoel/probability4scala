...
import com.markblokpoel.probability4scala.datastructures.{
  BigNatural,
  ProbabilityTree
}
...
case class Distribution[A](domain: Set[A], distribution: Map[A, BigNatural]) {
  ...
  def softmax(beta: BigNatural): Distribution[A] =
    softmax(beta, domain.nilDistribution)
  ...
  def entropy: BigNatural = {
    distribution.values.foldLeft(BigNatural(0))(
      (e: BigNatural, p: BigNatural) =>
        e - (if (p > 0) p * BigNatural.log(p) / BigNatural.log(2)
             else BigNatural(0)))
  }
  ...
  def klDivergence(that: Distribution[A]): BigNatural = {
    ...
    domain.foldLeft(BigNatural(0.0))(
      (kl: BigNatural, value: A) =>
...
case object Distribution {
  def apply[A, X: ClassTag](domain: Vector[A],
                            distribution: Vector[Double]): Distribution[A] =
    Distribution(domain.toSet,
                 (domain zip distribution.map(BigNatural(_))).toMap)

  def apply[A, X: ClassTag, Y: ClassTag](
      domain: Vector[A],
      distribution: Vector[BigNatural]): Distribution[A] =
    Distribution(domain.toSet, (domain zip distribution).toMap)

  def apply[A, X: ClassTag, Y: ClassTag](
      domain: Set[A],
      distribution: Map[A, Double]): Distribution[A] =
