...
object DistributionHelpers {
  def pr[A](value: A, distribution: Distribution[A]): BigNatural = distribution.pr(value)

  def pr[A, B](conditional: Conditional1[A, B], distribution: ConditionalDistribution[A, B]): BigNatural =
    distribution.pr(conditional)

  def prValue[A, B](value: A, distribution: ConditionalDistribution[A, B]): BigNatural = distribution.prValue(value)
